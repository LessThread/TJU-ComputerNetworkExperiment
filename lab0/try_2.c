...
int SEQNUM = 1;
int base;//开始位置
int nextseqnum;//最小未使用序号
int windows;//窗口长度
int B_last;
//struct msg Abuf[buf_size];//停等缓冲区，建议设置为50以上，这里设为100
struct pkt A_buf[buf_size];
...
A_output(struct msg message)
{
 ...
 if(nextseqnum>base+windows){
  ...
  //A端构造pkt
  struct pkt A_Send_B = generatePkg(message,UNDEFINE,nextseqnum);           //ACK初始假定为不确定，当NAK处理

  //向A缓冲区拷
  //strncpy(Abuf[SEQNUM%buf_size].data,message.data,20);
  A_buf[nextseqnum%buf_size]=A_Send_B;
...
B_input(struct pkt packet)
{
  ...
  struct msg message;
  PRINTF_DATA(buf," B_input:B get :");

//可能丢包，此过程需要连续性检验

  if(packet.checksum == generateChecksum(packet)&&packet.seqnum==B_last)
  {
  ...
  else if(packet.checksum != generateChecksum(packet))
  {
...
main()
{
   ...
   B_init();
    ...
    while (1) 
    {
        ...
        if (eventptr->evtype == FROM_LAYER5) //send
        { 
            if (nsim < nsimmax) 
            {
                if (nsim + 1 < nsimmax)
