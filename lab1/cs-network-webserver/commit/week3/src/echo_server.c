...
int splittingMessage()
{
    ...
    for(int i = 0;end_index<(strlen(all_request_buf));end_index++,i++)
    {
        ...
        printf("all_request_buf[%d]:%c\n",end_index,all_request_buf[end_index]);
        printf("sp: %d %d %d %d\n",sp[3] == '\r',sp[2] == '\n',sp[1] == '\r',sp[0] == '\n');

        if(end_index > 660){printf("r? %d n? %d\n",(all_request_buf[end_index]=='\r'),(all_request_buf[end_index]=='\n'));}
        if(((sp[3] == '\r')&&(sp[2] == '\n')&&(sp[1] == '\r')&&(sp[0] == '\n'))||((sp[0] == '\r')&&(sp[1] == '\n')&&(sp[2] == '\r')&&(sp[3] == '\n')))
        {
            printf("---sp\n");
            buf[i+1]='\0';
